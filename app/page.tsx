...
const AIRDROPPER = process.env.NEXT_PUBLIC_AIRDROPPER_ADDRESS as `0x${string}`;
...
type Row = { address: string; amountStr: string };
...
export default function Home() {
  ...
  async function connectWallet() {
    try {
      // @ts-ignore - MetaMask injects ethereum at runtime
      if (!window.ethereum) {
      ...
      setDecimals(dec);
      setStatus(`Connected: ${acc.slice(0, 6)}...${acc.slice(-4)} | Token decimals: ${dec}`);
    } catch (e: any) {
      console.error(e);
  ...
  function handleCSV(file: File | undefined | null) {
    if (!file) return;
    ...
    Papa.parse(file, {
      ...
      complete: (res: Papa.ParseResult<any>) => {
        try {
          const parsed: Row[] = res.data.map((row: any, idx: number) => {
  ...
  async function runAirdrop() {
    try {
      ...
      const erc20 = new ethers.Contract(TOKEN, ERC20_ABI, signer);
      const bal: bigint = await erc20.balanceOf(AIRDROPPER);
      const need: bigint = amounts.reduce((s, a) => s + a, 0n);
      if (bal < need) {
        setLoading(false);
